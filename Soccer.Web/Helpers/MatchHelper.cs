...
namespace Soccer.Web.Helpers
{
    public class MatchHelper : IMatchHelper
    {
        ...
        public MatchHelper(DataContext dataContext)
        {
            _dataContext = dataContext;
        }
        public async Task CloseMatchAsync(int matchId, int goalsLocal, int goalsVisitor)
        {
            _matchEntity = await _dataContext.Matches
                .Include(m => m.Local)
                .Include(m => m.Visitor)
        ...
        private void UpdatePositions()
        {
            ...
            GroupDetailEntity visitor = _matchEntity.Group.GroupDetails.FirstOrDefault(dg => dg.Team == _matchEntity.Visitor);
